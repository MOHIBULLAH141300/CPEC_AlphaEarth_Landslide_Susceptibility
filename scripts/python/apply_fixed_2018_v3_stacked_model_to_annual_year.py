...
def masked_stacked_prediction(
    ...
    x = clean_matrix(flat[flat_mask].copy(), bands, fills)
...
def initialize_nodata(dst: rasterio.io.DatasetWriter, block_size: int) -> None:
...
def apply_conventional(year: int, base_path: Path, mask: np.ndarray, fills: dict[str, float], block_size: int) -> dict[str, object]:
    ...
    with rasterio.open(base_path) as base_src, rasterio.open(LOCAL_V3) as local_src:
        ...
        with rasterio.open(out, "w", **output_profile(base_src)) as dst:
            total = len(range(0, base_src.height, block_size)) * len(range(0, base_src.width, block_size))
            for idx, win in enumerate(iter_windows(base_src.width, base_src.height, block_size), start=1):
                ...
                pred = masked_stacked_prediction(arr, CONVENTIONAL_BANDS, m, fills, base_models, meta_model)
                dst.write(pred, 1, window=win)
                if idx % 50 == 0 or idx == total:
                    print(f"{year} Conventional: {idx}/{total} windows", flush=True)
        ...
        with rasterio.open(out, "w", **output_profile(ref)) as dst:
            ...
            for tile_path in alpha_tiles:
                with rasterio.open(tile_path) as tile:
                    ...
                    total = len(range(0, tile.height, block_size)) * len(range(0, tile.width, block_size))
                    for idx, win in enumerate(iter_windows(tile.width, tile.height, block_size), start=1):
                        ...
                        pred = masked_stacked_prediction(arr, ALPHA_BANDS, m, fills, base_models, meta_model)
                        dst.write(pred, 1, window=dst_win)
                        if idx % 50 == 0 or idx == total:
                            print(f"{year} AlphaEarth tile {tile_path.name}: {idx}/{total} windows", flush=True)
            dst.set_band_description(1, cfg["description"])
            dst.update_tags(year=str(year), model="Spatial-CV Stacked Ensemble", feature_set=cfg["label"])
    return stats(out)
            ...
            for tile_path in alpha_tiles:
                with rasterio.open(tile_path) as tile:
                    ...
                    for idx, win in enumerate(iter_windows(tile.width, tile.height, block_size), start=1):
                        ...
                        pred = masked_stacked_prediction(arr, FUSED_BANDS, m, fills, base_models, meta_model)
                        dst.write(pred, 1, window=dst_win)
                        if idx % 50 == 0 or idx == total:
                            print(f"{year} Fused tile {tile_path.name}: {idx}/{total} windows", flush=True)
            dst.set_band_description(1, cfg["description"])
            dst.update_tags(year=str(year), model="Spatial-CV Stacked Ensemble", feature_set=cfg["label"])
    return stats(out)
