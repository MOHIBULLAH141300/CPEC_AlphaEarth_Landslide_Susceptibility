"""Generate reproducible, journal-ready figures for the manuscript-v3.2 analysis."""
...
PAPER_DIR = PROJECT_ROOT / "FINAL PAPER" / "Manuscript_single_file" / "v3_2_2026-07-17"
...
def figure_4_maps() -> None:
    ...
    fig=plt.figure(figsize=(11.8,7.5),constrained_layout=True)
    gs=GridSpec(3,2,figure=fig,width_ratios=[1.45,1.0],height_ratios=[1,1,0.76])
    ...
    ax_distribution.legend(fontsize=7.0,frameon=False,loc="upper right",ncol=1); panel_label(ax_distribution,"(d)")
...
def figure_6_robustness() -> None:
    ...
    y_delta=np.arange(len(robustness))[::-1]; offsets=[0.16,0.0,-0.16]; markers=["o","s","D"]
    for off,(column,label,color),marker in zip(offsets,metrics,markers):
        ax.scatter(robustness[column],y_delta+off,s=38,color=color,marker=marker,label=label,zorder=4)
    ax.axvline(0,color="#555555",lw=0.9)
    ax.set_yticks(y_delta,robustness.label)
    ax.set_xlabel("Difference from primary fused analysis")
...
def figure_7_transferability() -> None:
    ...
    ax.set_yticks(y,domains); ax.set_xlim(0.78,1.005); ax.set_xlabel("Held-out-domain metric (95% block-bootstrap CI)"); ax.grid(axis="x",color="#DDDDDD",lw=0.5)
    ax.legend(loc="lower center",bbox_to_anchor=(0.5,1.015),ncol=2,frameon=False,borderaxespad=0.0)
    panel_label(ax,"(f)")
...
def figure_8_aoa() -> None:
    ...
    ax=axes.flat[3]
    pivot=aoa.pivot(index="held_out_domain",columns="feature_set_name",values="aoa_coverage").reindex(index=domains,columns=feature_sets)
    matrix=ax.imshow(pivot.values,cmap=cmap,norm=norm,aspect="auto")
    for i in range(pivot.shape[0]):
        for j in range(pivot.shape[1]):
            value=float(pivot.iloc[i,j])
            ax.text(j,i,f"{100*value:.1f}%",ha="center",va="center",fontsize=8.2,fontweight="bold",color="white" if value<0.58 else "black")
    ax.set_yticks(np.arange(len(domains)),domains)
    ax.set_xticks(np.arange(len(feature_sets)),["Conventional","AlphaEarth\nEmbeddings","Conventional +\nAlphaEarth Embeddings"])
    ax.set_xlabel("Feature set")
    ax.set_ylabel("Held-out domain")
    ax.set_xticks(np.arange(-0.5,len(feature_sets),1),minor=True); ax.set_yticks(np.arange(-0.5,len(domains),1),minor=True)
    ax.grid(which="minor",color="white",lw=1.5); ax.tick_params(which="minor",bottom=False,left=False)
    panel_label(ax,"(d)")
    sm=mpl.cm.ScalarMappable(norm=norm,cmap=cmap); cbar=fig.colorbar(sm,ax=list(axes.flat),orientation="horizontal",fraction=0.04,pad=0.04); cbar.set_label("Harmonised AoA coverage by held-out domain")
...
def figure_9_road_exposure() -> None:
    ...
    fig=plt.figure(figsize=(12.2,8.5),constrained_layout=True)
    gs=GridSpec(3,2,figure=fig,width_ratios=[1.55,1.0],height_ratios=[1.0,0.62,0.86])
    ax_network=fig.add_subplot(gs[0:2,0]); ax_kkh=fig.add_subplot(gs[0,1]); ax_share=fig.add_subplot(gs[1,1])
    routes_gs=gs[2,:].subgridspec(1,2,width_ratios=[1.45,1.0],wspace=0.08)
    ax_routes_len=fig.add_subplot(routes_gs[0,0]); ax_routes_score=fig.add_subplot(routes_gs[0,1],sharey=ax_routes_len)

    terrain_background(ax_network,dem,dem_extent)
    ax_network.add_patch(Rectangle((dem_extent[0],dem_extent[2]),dem_extent[1]-dem_extent[0],dem_extent[3]-dem_extent[2],facecolor="white",edgecolor="none",alpha=0.32,zorder=2))
    plot_domains(ax_network,alpha=0.08)
    ax_network.add_collection(LineCollection(line_segments,colors="white",linewidths=1.35,alpha=0.65,zorder=9))
    collection=LineCollection(line_segments,cmap="magma",norm=score_norm,linewidths=0.95,zorder=10); collection.set_array(segments.fused_score.to_numpy()); ax_network.add_collection(collection)
    plot_boundary(ax_network,lw=0.8); ax_network.set_xlim(59.8,80.6); ax_network.set_ylim(22.9,42.0); ax_network.set_aspect("equal"); ax_network.set_anchor("N")
    ax_network.set_xlabel("Longitude (deg E)"); ax_network.set_ylabel("Latitude (deg N)"); panel_label(ax_network,"(a)")
    c=fig.colorbar(collection,ax=ax_network,fraction=0.040,pad=0.02); c.set_label("Case-control susceptibility score")
    ...
    class_colors={"below_p90":"#7A7A7A","supported_high_score":"#009E73","verification_priority_high_score":"#D55E00"}; class_labels={"below_p90":"Below P90","supported_high_score":"Supported high","verification_priority_high_score":"Verification priority"}
    ...
    ax_kkh.set_xlim(72.3,79.2); ax_kkh.set_ylim(33.2,40.7); ax_kkh.set_aspect("equal"); ax_kkh.set_anchor("N")
    ax_kkh.set_xlabel("Longitude (deg E)"); ax_kkh.set_ylabel("Latitude (deg N)"); panel_label(ax_kkh,"(b)")
    ax_kkh.legend(handles=[Line2D([0],[0],color=class_colors[p],lw=2,label=class_labels[p]) for p in class_colors],loc="lower center",bbox_to_anchor=(0.5,1.015),ncol=3,fontsize=6.8,frameon=False,borderaxespad=0.0)

    groups=["Full CPEC road network","KKH proxy route (N35/314)"]
    d=summary.set_index("road_group").reindex(groups); total=d.total_length_km.to_numpy(); pct80=100*d.actual_length_ge_p80_km.to_numpy()/total; pct90=100*d.actual_length_ge_p90_km.to_numpy()/total
    y_share=np.arange(2)[::-1]
    for yy,p80,p90 in zip(y_share,pct80,pct90):
        ax_share.plot([p90,p80],[yy,yy],color="#A7A7A7",lw=3.0,solid_capstyle="round",zorder=1)
    ax_share.scatter(pct80,y_share,s=58,color="#E69F00",marker="o",label=">= P80",zorder=3)
    ax_share.scatter(pct90,y_share,s=58,color="#D55E00",marker="D",label=">= P90",zorder=3)
    ax_share.set_yticks(y_share,["Full network","KKH proxy"]); ax_share.set_xlim(0,100); ax_share.set_xlabel("Share of road length (%)")
    ax_share.grid(axis="x",color="#E2E2E2",lw=0.55,zorder=0); ax_share.legend(loc="lower center",bbox_to_anchor=(0.5,1.02),ncol=2,frameon=False,borderaxespad=0.0); panel_label(ax_share,"(c)")

    routes=routes.sort_values("actual_length_ge_p90_km"); y=np.arange(len(routes))
    for yy,length in zip(y,routes.actual_length_ge_p90_km):
        ax_routes_len.plot([0,length],[yy,yy],color="#B8C7D9",lw=2.2,solid_capstyle="round",zorder=1)
    ax_routes_len.scatter(routes.actual_length_ge_p90_km,y,s=46,color="#3F78A8",edgecolor="white",linewidth=0.5,zorder=3)
    ax_routes_len.set_yticks(y,routes.route_label); ax_routes_len.set_xlim(0,float(routes.actual_length_ge_p90_km.max())*1.10)
    ax_routes_len.set_xlabel("Actual road length >= map P90 (km)"); ax_routes_len.grid(axis="x",color="#E2E2E2",lw=0.55,zorder=0); panel_label(ax_routes_len,"(d)")
    ax_routes_score.scatter(routes.length_weighted_mean_score_no_road_distance,y,s=48,color="#D55E00",marker="D",edgecolor="white",linewidth=0.5,zorder=3)
    ax_routes_score.set_xlim(0,1); ax_routes_score.set_xlabel("Mean score without road distance"); ax_routes_score.grid(axis="x",color="#E2E2E2",lw=0.55,zorder=0)
    ax_routes_score.tick_params(axis="y",left=False,labelleft=False); ax_routes_score.spines["left"].set_visible(False)
