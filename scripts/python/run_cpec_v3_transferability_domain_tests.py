...
OUT = PROJECT / "03_models" / "v3_transferability_domain_tests"
FIG_DIR = PROJECT / "05_reports" / "figures" / "v3_transferability_domain_tests"
PACKAGE_METHODS = PACKAGE / "01_methods_and_decisions"
PACKAGE_TABLES = PACKAGE / "02_model_performance_tables"
PACKAGE_FIGS = PACKAGE / "03_figures" / "05_transferability_domain_tests"
...
FEATURE_SETS = {
    ...
    "Conventional + AlphaEarth": CONVENTIONAL + ALPHA,
}
BASE_KEYS = ["logistic_l2", "random_forest", "extra_trees", "xgboost", "lightgbm", "catboost"]
DOMAIN_ORDER = [
    "Northern high mountains / KKH-Kashgar",
    "Western Pakistan / Balochistan",
    "Indus lowland and central corridor",
...
def assign_domain(df: pd.DataFrame) -> pd.Series:
    def domain(row: pd.Series) -> str:
        lon, lat, elev = row["longitude"], row["latitude"], row["elevation_m"]
        if lat >= 34.0 or elev >= 2200:
            return "Northern high mountains / KKH-Kashgar"
        if lat < 26.5:
            return "Southern/coastal corridor"
        if lon < 68.5:
            return "Western Pakistan / Balochistan"
        if elev < 700 and lon >= 68.5:
            return "Indus lowland and central corridor"
        return "Transitional foothills and plateaus"

    return df.apply(domain, axis=1)
...
def run_one_transfer(
    ...
    source = df[df["cpec_transfer_domain"] != target_domain].copy()
    target = df[df["cpec_transfer_domain"] == target_domain].copy()
    ...
    preds = target[
        ["longitude", "latitude", "label", "spatial_fold_5", "cpec_transfer_domain"]
    ].copy()
...
def make_figures(df: pd.DataFrame, metrics_df: pd.DataFrame, shift_df: pd.DataFrame) -> None:
    ...
    # 1. Domain sample map
    colors = {
        "Northern high mountains / KKH-Kashgar": "#2563eb",
        "Western Pakistan / Balochistan": "#b45309",
        "Indus lowland and central corridor": "#059669",
        "Southern/coastal corridor": "#dc2626",
        "Transitional foothills and plateaus": "#7c3aed",
    }
    fig, ax = plt.subplots(figsize=(8.4, 8.0))
    for domain in DOMAIN_ORDER:
        sub = df[df["cpec_transfer_domain"] == domain]
        ax.scatter(
            sub["longitude"],
            sub["latitude"],
    ...
    ax.set_title("CPEC transferability subdomains and 2018 sample distribution", fontweight="bold")
    ax.grid(True, color="#e5e7eb", linewidth=0.6)
    ax.legend(loc="lower right", fontsize=7, frameon=True)
    fig.tight_layout()
    save_all(fig, "figure_cpec_v3_transferability_subdomain_sample_map")
    ...
    fig.tight_layout()
    save_all(fig, "figure_cpec_v3_transferability_stacked_auc_heatmap")
    ...
    fig.tight_layout()
    save_all(fig, "figure_cpec_v3_domain_shift_vs_transfer_auc")


def short_domain(label: str) -> str:
    return {
        "Northern high mountains / KKH-Kashgar": "KKH",
        "Western Pakistan / Balochistan": "West",
        "Indus lowland and central corridor": "Indus",
        "Southern/coastal corridor": "South",
        "Transitional foothills and plateaus": "Foothills",
    }.get(label, label[:8])
...
def save_all(fig, stem: str) -> None:
    ...
    plt_close = getattr(fig, "clf", None)
    if plt_close:
        pass
...
def write_report(domain_balance: pd.DataFrame, metrics_df: pd.DataFrame, base_df: pd.DataFrame, shift_df: pd.DataFrame) -> None:
    ...
    balance_md = domain_balance.to_markdown(index=False)
    report = f"""# CPEC V3 Transferability Domain Tests

Date: 2026-05-10

## Purpose

This analysis implements the first major step of the teacher-approved updated methodology: **transferability-aware landslide susceptibility modelling**. It tests whether the 2018 V3 model framework transfers across major CPEC subdomains rather than relying only on random or ordinary spatial folds.

## Domain Definition

Domains were assigned from available sample longitude, latitude, and elevation:

- Northern high mountains / KKH-Kashgar: latitude >= 34.0 or elevation >= 2200 m.
- Southern/coastal corridor: latitude < 26.5.
- Western Pakistan / Balochistan: longitude < 68.5, excluding southern/coastal and northern/high-mountain samples.
- Indus lowland and central corridor: longitude >= 68.5 and elevation < 700 m, excluding southern/coastal and northern/high-mountain samples.
- Transitional foothills and plateaus: remaining samples.

This is a first operational domain scheme. It can later be replaced or refined with formal geomorphic units if a better CPEC geomorphic map becomes available.
...
- Domain-level transfer metrics are more rigorous than random-split accuracy because they test extrapolation to different geomorphic/infrastructure settings.
...
- Metrics: `v3_transferability_leave_one_domain_metrics.csv`
- Base learner metrics: `v3_transferability_base_learner_metrics.csv`
- Domain shift table: `v3_transferability_domain_shift.csv`
- Predictions: `v3_transferability_held_out_predictions.csv`
- Domain sample table: `cpec_2018_lsm_samples_v3_with_transfer_domains.csv`
- Figures:
  - `figure_cpec_v3_transferability_subdomain_sample_map.png`
  - `figure_cpec_v3_transferability_stacked_auc_heatmap.png`
  - `figure_cpec_v3_domain_shift_vs_transfer_auc.png`
...
"""
    for path in [
        OUT / "v3_transferability_domain_tests_report.md",
        PACKAGE_METHODS / "v3_transferability_domain_tests_report_2026-05-10.md",
    ]:
...
def main() -> None:
    ...
    df["cpec_transfer_domain"] = assign_domain(df)
    domain_balance = (
        df.groupby("cpec_transfer_domain")["label"]
    ...
    domain_balance = domain_balance.set_index("cpec_transfer_domain").reindex(DOMAIN_ORDER).reset_index()
    domain_balance.to_csv(OUT / "v3_transferability_domain_sample_balance.csv", index=False)
    df.to_csv(OUT / "cpec_2018_lsm_samples_v3_with_transfer_domains.csv", index=False)
    df.to_csv(PACKAGE_TABLES / "cpec_2018_lsm_samples_v3_with_transfer_domains.csv", index=False)
    ...
    metrics_df.to_csv(OUT / "v3_transferability_leave_one_domain_metrics.csv", index=False)
    base_df.to_csv(OUT / "v3_transferability_base_learner_metrics.csv", index=False)
    pred_df.to_csv(OUT / "v3_transferability_held_out_predictions.csv", index=False)
    shift_df.to_csv(OUT / "v3_transferability_domain_shift.csv", index=False)
    (OUT / "v3_transferability_stacked_meta_coefficients.json").write_text(
        json.dumps(meta_records, indent=2), encoding="utf-8"
    )

    for src in [
        "v3_transferability_leave_one_domain_metrics.csv",
        "v3_transferability_base_learner_metrics.csv",
        "v3_transferability_domain_shift.csv",
        "v3_transferability_domain_sample_balance.csv",
    ]:
