"""Run a reproducible structural and terminology audit of the v3 manuscript."""
...
def run_audit(docx: Path, report: Path) -> str:
    ...
    for match in re.finditer(r"\bprobabilit(?:y|ies)\b", text, flags=re.IGNORECASE):
        left = max(0, match.start() - 90)
        right = min(len(text), match.end() + 120)
        probability_contexts.append(re.sub(r"\s+", " ", text[left:right]).strip())
    ...
    checks = {
        ...
        "seven_tables_numbered_1_to_7": table_numbers == expected,
        "seven_figures_numbered_1_to_7": figure_numbers == expected,
    ...
    lines = [
        "# Manuscript v3 final consistency audit",
        "",
        f"- File: `{docx}`",
    ...
    if stale:
        ...
        lines.append("- None")
    lines.extend(["", "## Probability wording contexts", ""])
    if probability_contexts:
        lines.extend(f"- {context}" for context in probability_contexts)
    else:
        lines.append("- None")
